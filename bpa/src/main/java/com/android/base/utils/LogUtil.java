...
public class LogUtil {
    ...
    public static void json(String method, String json) {
        if (IS_SHOW_LOG) {
            ...
            String log = formatStr(method, json);
            if (log.length() > 4000) {
                for (int i = 0; i < log.length(); i += 4000) {
                    if (i + 4000 < log.length()) {
                        Log.i(TAG, log.substring(i, i + 4000));
                    } else {
                        Log.i(TAG, log.substring(i, log.length()));
                    }
                }
            } else {
                Log.i(TAG, log);
    ...
    /**
     * 输出日志相关信息拼接 at
     * com.android.base.activity.MainActivity.init(MainActivity.java:58)
     * <p>
     * <br> Version: 1.0.0
